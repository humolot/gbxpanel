...
15. [API](#api)
16. [Backup storage](#backup-storage)
17. [Clients](#clients)
...
| **Optimization** | Performance settings of PHP-FPM, Apache and MySQL with plans by memory, values measured on this server, live status and advice, and automatic rollback when a service refuses them; see [Optimization](#optimization). |
| **API** | REST API with keys, permissions, rate limit, idempotent writes, signed webhooks, call log and an OpenAPI document; see [API](#api). |
| **Backup** | Remote destinations for the backups of the panel (object storage, Google Drive, Dropbox, OneDrive, pCloud, Box, FTP, SFTP, WebDAV), transfers with their logs, and the backups already stored at each destination; see [Backup storage](#backup-storage). |
| **Clients** | Hosting customers with packages, quotas, suspension and a client sub-panel (Overview, Website, FTP, Database, Security); see [Clients](#clients). |
...
**Home > Software** has an **Optimization** button on PHP, Apache and MySQL. It shows the values the service is running with, values proposed for the memory of this machine, and a status panel with advice.

| Component | What can be changed |
| --- | --- |
| PHP-FPM (per version) | Process manager, number of workers, idle workers, requests before a worker restarts, request timeout, slow request log, and OPcache (memory, files kept, timestamp checks, JIT) |
| Apache (event MPM) | Connections at the same time, threads per process, processes, idle threads, requests before a process restarts, request timeout and keep-alive |
| MySQL / MariaDB | InnoDB buffer pool and log file, temporary tables, connections and thread cache, open tables cache, per connection buffers, write safety and flush method, disk operations per second, slow query log |

- **The proposal is measured, not guessed.** For PHP the panel reads how much memory the running workers of that version actually use and derives the number of workers from it, instead of assuming a fixed size per process. The memory plan (1-2 GB, 2-4 GB, ...) only decides which machine the numbers are calculated for; "This server" uses what is really installed.
- **Nothing is applied blindly.** The file is copied first, the service validates the new configuration (`php-fpm -t`, `apache2ctl configtest`), and if the check or the reload fails the previous file comes back and the service is reloaded with it. Values that would keep a service from starting (more idle workers than workers, fewer Apache processes than the connections need) are refused before anything is written.
- **MySQL is never edited in the files of the distribution:** the panel writes `/etc/mysql/conf.d/gbx-tuning.cnf`, which both MySQL and MariaDB read. Settings that can change while the server runs are applied live, and the answer says which ones still need the restart button.
- **Status and advice:** memory per PHP worker and what the workers would use if all of them were busy; Apache processes, memory and MPM in use; for MySQL the queries per second, connections against the limit, buffer pool against the size of the data, hit rate and slow queries, each with a sentence explaining what to do about it.
- **Disabled functions** (PHP): the list of functions PHP refuses to run, with a recommended set for shared servers. The functions the panel itself needs are never blocked.

## API
Everything the panel does can also be done from outside it: `https://<panel address>/api/v1`. The API drives the same code as the interface, so an action through the API behaves exactly like the same action in the panel, including the activity log. **API** is an administrator page with four tabs: Keys, Webhooks, Log and Documentation.

**Authentication.** `Authorization: Bearer <token>`. A key is shown once when it is created; the panel stores only its SHA-256 hash. Every key carries:

| Setting | What it does |
| --- | --- |
| Permissions | Scopes such as `websites:read`, `websites:write`, `clients:write`; `websites` grants read and write of that group, `*` grants everything |
| Allowed addresses | List of IPs or CIDR ranges; calls from anywhere else are refused |
| Requests per minute | Rate limit of that key; answers carry `X-RateLimit-Limit` and `X-RateLimit-Remaining` |
| Expiration | The key stops working on that date |
| Client | A key bound to a client only reaches the resources of that client |

**Answers.** Always JSON with `ok`. Success adds `data` (and `meta` with `page`, `per_page`, `total` on lists); a failure adds `error.code`, `error.message` and, for invalid input, `error.fields`. Every answer carries `X-Request-Id`, the same value shown in the Log tab. Actions that take longer answer with a background task; follow it on `GET /tasks/{id}`.

**Repeat-safe writes.** Send an `Idempotency-Key` header and a repeated call answers the first result (header `Idempotent-Replay: true`) instead of creating the same website twice - the usual problem when a client retries after a timeout.

```bash
# list websites
curl -s https://panel.example.com:8888/api/v1/websites \
  -H "Authorization: Bearer YOUR_KEY"

# create one, safely repeatable
curl -s -X POST https://panel.example.com:8888/api/v1/websites \
  -H "Authorization: Bearer YOUR_KEY" -H "Content-Type: application/json" \
  -H "Idempotency-Key: 6f1b2c7a" \
  -d '{"domain":"example.com","php_version":"8.3","add_www":true,"create_database":true}'
```

**Endpoints** (the Documentation tab lists every one with its parameters and required permission): system status, services and metrics; websites with their settings sections, SSL, backups, logs and usage; databases with backups and restore; FTP accounts; DNS zones and records; clients, packages and their resources; backups, storages and transfers; scheduled tasks; files; firewall and Fail2ban; background tasks; webhooks. `GET /api/v1/openapi.json` returns the OpenAPI 3.1 document, generated from the same list the routes come from, ready to import in Postman or a client generator.

**Webhooks.** Instead of asking the panel in a loop, let it call you: website created, deleted, started or stopped, SSL issued, database created, client created, suspended or reactivated, backup and transfer finished or failed, task finished or failed, malware found. Every message is signed - the header `X-GBX-Signature` carries `sha256=HMAC-SHA256(secret, body)` - and a delivery that fails is repeated five times with growing pauses (1, 5, 30 and 120 minutes), with the whole history visible in the panel.

**Log.** Every call is recorded with key, method, path, status, duration and address, and can be filtered to failures only.

## Backup storage

Backups are made on this server (Cron jobs, website backups and the automatic database backup) and can then be sent to a destination outside it. The transfers are done by [rclone](https://rclone.org), installed by the installer and by `gbx update`; the panel writes a root-only configuration for each operation, so credentials never appear on a command line, and reads back the OAuth tokens rclone refreshed. **Backup** is an administrator page.

| Group | Destinations |
| --- | --- |
| Object storage | Amazon S3, DigitalOcean Spaces, Linode (Akamai), Wasabi, Backblaze B2, Cloudflare R2, Vultr, Hetzner, Contabo, Scaleway, OVHcloud, Google Cloud Storage, Azure Blob Storage and any other S3 compatible service (MinIO, Ceph...) |
| Cloud drives | Google Drive (including shared drives), Dropbox, Microsoft OneDrive, pCloud, Box |
| Remote servers | SFTP (SSH), FTP and FTPS, WebDAV and Nextcloud |

- **Storage** tab: destinations with their status, root path, space used and last check. A destination is only saved when rclone can write, read back and delete a test file with it. Credentials are stored encrypted; passwords of FTP, SFTP and WebDAV are kept in the format rclone expects, never in clear text. **Browse** lists what is stored at the destination, with download to your browser, download to this server, restore and delete.
- **Google Drive** connects in two ways: **Connect with Google** (the panel runs the authorization; Google only accepts HTTPS redirect URIs, so the panel must be opened through a domain with SSL and the redirect URI shown in the form must be added to the OAuth client), or **paste code** (run the `rclone authorize "drive"` command shown in the form on a computer with a browser and paste the answer in the Token field). The other cloud drives use the paste method. The default access asks only for the files the panel creates.
- **Transfers** tab: every upload and download with its state, progress, log, retry, cancel and history. Transfers run in their own process, outside the queue worker, so a transfer of several hours is not interrupted by a job timeout and continues after the browser is closed.
- **Local backups** tab: the backups in `/www/backup`, with **Send to storage**.
- **Settings** tab: webhook called when a transfer fails (and optionally when one finishes), bandwidth limit, part size, how many transfers run at the same time and how long the history is kept. Failures are always written to the activity log.

**Large backups.** Object storage and the cloud drives receive files in chunks with their own retries, so a hiccup does not restart a 10 GB upload. FTP, SFTP, WebDAV and pCloud have no resumable uploads: archives larger than the part size (1 GB by default) are split, sent into a `<name>.parts` folder and verified afterwards, so a repeated transfer only sends the parts that are missing. Every backup is uploaded with a `.sha256` file next to it, which is checked when the backup is downloaded again. Transfers can be limited in bandwidth and run with low CPU and disk priority.

**Where the backups land.** Inside the folder of the destination: `site/<domain>/`, `database/<engine>/<database>/` and `path/<label>/`. The retention at the destination is separate from the local one, so you can keep 3 copies here and 30 there.

**Using a destination:**
- **Cron jobs** (website, database and directory backups): *Backup to* chooses the destination, *Local copy* can delete the local file after a successful upload, and *Remote copies* sets the retention at the destination. Website backups also upload the dumps of the databases linked to the site.
- **Databases > Automatic backup:** the same three options for the daily dump of every database.
- **Websites > Backups:** *Send to* uploads the archive (and its dumps) when the backup finishes.

**Restoring:** *Download to server* brings the backup back to `/www/backup`, where the usual restore works. *Restore* in the Browse window does both steps: the backup is downloaded, its checksum is verified and the restore of the website or database it came from starts as a task.

**Commands:** `gbx:backup-push` (used by the scheduled jobs; uploads a file and waits for it), `gbx:backup-transfer` (runs one queued transfer) and `gbx:backup-housekeeping` (marks interrupted transfers, deletes old history and, with `--usage`, measures the space used at every destination). The last one runs hourly and daily from the scheduler.

## Clients

**Clients** (administrators) sells hosting to customers who manage their own resources in a client sub-panel with the same interface.

| Tab | Features |
| --- | --- |
| Account | Username, package, e-mail, disk and bandwidth used against the limits, resources, status, expiration and remarks. **Login Sub Panel** opens the sub-panel as the client (recorded in the log, "Back to admin" returns). Edit, resources (assign websites, databases, FTP accounts, cron jobs and DNS zones; databases and FTP accounts of a website follow it), suspend with a reason, reactivate, reset two-factor authentication and delete (resources are kept and return to the administrator) |
| Package | Websites, databases, FTP accounts, disk (MB), monthly bandwidth (MB), allowed PHP versions and Let's Encrypt SSL; 0 means unlimited |
| Storage | Disk and bandwidth per client with the website folders; measure now |
| Logs | Everything clients did (and what administrators did while signed in as a client) |
| Settings | Enable the sub-panel, its name, and what happens when an account expires, the disk quota is full or the bandwidth is used up (block new resources, suspend or only show the usage) |

**Client sub-panel** at `https://<panel address>/client`:
- Sign-in with its own session (guard `client`), rate limiting and optional two-factor authentication. The administrator panel stays behind the security entrance and IP allow list; the sub-panel does not need them and answers only while the sub-panel is enabled and at least one client exists.
- **Overview:** today's requests, websites/FTP/databases against the package, requests over 24 hours, bandwidth this month and disk usage with 30 day charts.
- **Website:** create within the package (the document root is always `/www/wwwroot/<domain>`; a domain or alias already used by another website is refused), start/stop, PHP version from the package, Let's Encrypt SSL, access and error logs, delete (optionally with files, databases and FTP accounts).
- **FTP:** accounts limited to a folder of one of the client's websites; password, enable/disable, delete.
- **Database:** MySQL databases whose name and user start with a prefix unique to the client (`<username><id>_`), so a client can never take over another database user; connection details, password, delete. **phpMyAdmin** opens already signed in with the database user (server 2 with sign-on authentication, added to phpMyAdmin's configuration automatically). **Backup / Import:** backup now (the newest 5 are kept), download, restore and delete backups, export a fresh `.sql.gz` and import `.sql`, `.sql.gz` or `.zip` files. Imports and restores run with the client's database user through a temporary option file, never as the MySQL root user.
- **File:** file manager for the folders of the client's websites: browse, create, upload (drag and drop), download, the same code editor as the administrator (tabs, search in files, settings) limited to the client websites, rename, copy, move, delete, compress (.zip, .tar.gz) and extract. Paths are resolved through symbolic links before every operation, operations run as the website user instead of root, and links that point outside the website are removed after an archive is extracted. Uploads, copies and extraction stop while the disk quota is full.
- **Security:** password and two-factor authentication.

Every request checks that the resource belongs to the signed-in client (other ids answer 404). Suspending an account stops its websites and disables its FTP accounts; reactivating restarts only what the suspension stopped. `gbx:clients-usage` runs hourly (requests and bytes of the previous hours from the access logs) and daily with `--disk` (website folders and MySQL database sizes), then applies expiration and the quota actions.

> Isolation: in this version client websites run with the shared web user, like every other website. Give accounts to trusted customers until per-client isolation (Linux user, PHP-FPM pool with open_basedir, FTP and cron as that user) is enabled; the database already has the fields for it (`clients.system_user`, `clients.isolated`).
## Architecture
