...
| **Terminal** | Root command console with a persistent working directory and history (non-interactive). |
...
│                              BackupManager, ClamAvManager, TaskRunner, TaskHooks, AiAssistant
...
| Component | Details |
| --- | --- |
| Web | Apache virtual host on the panel port → PHP-FPM pool `gbxpanel` (`/run/php/gbxpanel.sock`) running as user `gbxpanel` |
